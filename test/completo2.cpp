...
extern "C" void app_main()
{
    ...
    while (1)
    {
        ...
        if (current_time - prev_time_calculations >= 1000000) // Procesar cada segundo
        {
        ...
        //    joint_variables[0], joint_variables[1], joint_variables[2], joint_variables[3], j3_motor.getError(), j4_motor.getError());
            ...
            if (message_length)
            {
                ...
                // {
                //     j1.setMaxSpeed(4000);
                //     j1.setAcceleration(2000);

                //     j2.setMaxSpeed(4000);
                //     j2.setAcceleration(2000);
                // }
                ...
                else if (mode == 1) // Cinemática Directa
                {
                    ...
                    if (areDifferent)
                        for (int i = 0; i < 2; ++i)
                        {
                            ...
                            j1_motor.move((steppers_targets[0] / 360) * 200 * 25);
                            ...
                            {
                                j1_motor.run(); // Ejecutar el movimiento
                            }

                            while(j3_motor.isRunning() || j4_motor.isRunning())
                            {
                            j3_motor.update();
                            j4_motor.update();
                            }
            ...
            prev_time = current_time; // Actualizar el tiempo previo
            // printf("Tiempo de cálculo y Bluetooth: %lld microsegundos\n", esp_timer_get_time() - start_time);
        }
    }
}
