...
extern "C" void app_main()
{
    ...
    while (1)
    {
        ...
        // if (current_time - prev_time >= 62.5)
        // {
        //     j3_motor.update();
        //     j4_motor.update();
        ...
        if (current_time - prev_time >= 10000)
        {
            ...
            {
                j1_motor.run();
                printf("Pos Motor: %d\n", j1_motor.currentPosition());
                printf("Vel Motor: %f\n", j1_motor.speed());
            }

            while (j2_motor.isRunning())
            {
                j2_motor.run();
                printf("Pos Motor: %d\n", j2_motor.currentPosition());
                printf("Vel Motor: %f\n", j2_motor.speed());
            }
        ...
        if (current_time - prev_time_calculations >= 100000)
        {
            ...
            if (message_length)
            {
                ...
                printf("theta1: %.2f, theta2: %.2f, Z: %.2f, theta3 = %f, Error3: %.2f, Error 4: %.2f, Reference: %d\n",
                       joint_variables[0], joint_variables[1], joint_variables[2], joint_variables[3], j3_motor.getError(), j4_motor.getError(), static_cast<int>(round((steppers_targets[0] / 360) * 5000)));
            }
