...
const uint8_t AS5600_UNUSED_GPIO = 255;
...
{
public:
    AS5600(i2c_port_t i2c_num = I2C_NUM_0, uint8_t address = 0x36, uint32_t freq = 400000); //De 100kHz (Standard) 400 kHz (Fast) 1MHz (No probado)
    ~AS5600();

    bool setup(uint8_t sdaPin = 21, uint8_t sclPin = 22, uint8_t directionPin = AS5600_UNUSED_GPIO);
...
protected:
    ...
    esp_err_t readReg2(uint8_t reg, uint16_t &data);
    esp_err_t writeReg(uint8_t reg, uint8_t value);
    esp_err_t writeReg2(uint8_t reg, uint16_t value);
...
{
public:
    AS5600L(i2c_port_t i2c_num = I2C_NUM_0, uint8_t address = AS5600L_DEFAULT_ADDRESS);
