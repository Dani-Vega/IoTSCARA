...
Serial::uart_obj_t Serial::uart_obj = {};

Serial::Serial(){}

Serial::~Serial(){}

void Serial::tx_task(void *pvParameters)
{
...
void Serial::rx_task(void *pvParameters)
{
	ESP_LOGI(pcTaskGetName(0), "Start");
	uart_obj.rx_len = 0;
...
void Serial::setup(int baudrate, int txd, int rxd) {
...
void Serial::println(char *ch) {
	...
	_ch[len] = 0x0d;
	_ch[len+1] = 0x0a;
	write_buffer((uint8_t *)_ch, len+2);
}
...
void Serial::write_buffer(uint8_t *ch, int len) {
	for (int i=0;i<len;i++) {
