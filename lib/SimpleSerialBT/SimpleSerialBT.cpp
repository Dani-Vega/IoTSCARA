...
const char* SerialBT::getMacAddress()
{
    static char macStr[18]; // Dirección MAC formateada como cadena (17 caracteres + nulo)
    uint8_t mac[6];         // Array para almacenar los 6 bytes de la dirección MAC

    // Obtener la dirección MAC
    esp_err_t ret = esp_read_mac(mac, ESP_MAC_BT);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Error al obtener la dirección MAC: %s", esp_err_to_name(ret));
        return nullptr; // Retornar nulo en caso de error
    }

    // Formatear la dirección MAC en formato estándar (XX:XX:XX:XX:XX:XX)
    snprintf(macStr, sizeof(macStr), "%02X:%02X:%02X:%02X:%02X:%02X", 
             mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    ESP_LOGI(TAG, "Dirección MAC: %s", macStr);
    return macStr; // Retornar la dirección MAC como cadena
}
